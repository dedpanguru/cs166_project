...
func LoginHandler(res http.ResponseWriter, req *http.Request) {
	...
	result, err := database.Retrieve(input["username"])
	if result == nil {
		// new user with a token doesn't make sense, return 403 Forbidden
		http.Error(res, "How did you get a token?", http.StatusUnauthorized)
		return
	} else if err != nil {
		// if there was an error, let the client know
		http.Error(res, err.Error(), http.StatusInternalServerError)
	...
	if result.Token == nil {
		...
		err2 := ValidateJWT([]byte(result.Token))
		if err2.Error() == "Token expired" {
			// token expired case
			refreshNeeded = true
		} else {
